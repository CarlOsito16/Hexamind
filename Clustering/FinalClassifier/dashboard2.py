...
#TAB1
score_chart_df = chart_df(all_comp_df)
category_orders = {'company' : ['carrefour' , 'auchan', 'leclerc', 'lidl']}
score_chart = px.histogram(data_frame=score_chart_df,
...
score_chart.update_traces(textposition='outside',
                    )
score_chart.update_layout(xaxis_title = 'Topics',
...
tab1, tab2 = st.tabs(['Holistic view', 'breakdown'])


with tab1:
    st.write("""
            # Holistic view of *Carrefour* ratings compared to three major competitors
            """
    )

    tab1_row1_col1 , tab1_row1_col2, tab1_row1_col3,  tab1_row1_col4 =  st.columns([1,1,1,1])
    
    with tab1_row1_col1:
        st.image("UI-element/carrefour logo.png", width = 100)
        st.metric(label = 'Overall Ratings', 
                  value = score_chart_df[(score_chart_df['topic'] == 'All topics') &
                                         (score_chart_df['company'] == 'carrefour') ].score )
        
        
        
        
    with tab1_row1_col2:
        st.image("UI-element/auchan_logo.png", width = 100)

    with tab1_row1_col3:
        st.image("UI-element/leclerc_logo.png", width = 100)
        
    with tab1_row1_col4:
        st.image("UI-element/lidl_logo.png", width = 100)
