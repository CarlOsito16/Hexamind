...
if "history_1" not in st.session_state:
    ...
    st.session_state.history_1.append({"message": "how can I help you today?", "is_user": False} )
    st.session_state.history_1.append({"message": "Tell me more about you", "is_user": True} )
...
def send_message():
    ...
    st.session_state.history_1.append({"message": input_value  , "is_user": False})
    st.session_state.history_1.append({"message": f"{np.random.choice(pre_questions)} + {np.random.randint(low= 0,high =100, size=1)}"   , "is_user": True})
