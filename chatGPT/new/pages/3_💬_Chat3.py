...
st.write("## 💬 Conversation 3")
if "history_3" not in st.session_state:
    st.session_state.history_3 = []
    st.session_state.history_3.append({"message": "how can I help you today?", "is_user": False} )
    st.session_state.history_3.append({"message": "Tell me more about you", "is_user": True} )
...
def send_message():
    ...
    st.session_state.history_3.append({"message": input_value  , "is_user": False})
    st.session_state.history_3.append({"message": f"{np.random.choice(pre_questions)} + {np.random.randint(low= 0,high =100, size=1)}"   , "is_user": True})
...
with row1_col1:
    ...
    for chat in st.session_state.history_3:
        st_message(**chat)
